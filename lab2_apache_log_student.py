...
dayToHostPairTuple = access_logs.map(lambda x:((x.date_time.day, x.host),1))

dayGroupedHosts = dayToHostPairTuple.reduceByKey(lambda x,y:1)

dayHostCount = dayGroupedHosts.map(lambda (a,b):(a[0],1)).reduceByKey(lambda x,y:x+y)
...
avgDailyReqPerHost = (sortedByDay
                      .zip(dailyHosts).map(lambda (x,y):(x[0],x[1]/y[1]))).cache()
...
badUniqueEndpoints = badEndpoints.reduceByKey(lambda x,y:1).map(lambda x:x[0])
...
badEndpointsTop20 = badEndpointsSum.takeOrdered(20,lambda (x,y):-y)

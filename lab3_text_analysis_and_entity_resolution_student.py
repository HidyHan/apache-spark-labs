...
def simpleTokenize(string):
    ...
    """
    l=[i.lower() for i in re.split(split_regex,string) if i != ""]
    return l
...
def tokenize(string):
    ...
    """
    return [i.lower() for i in re.split(split_regex,string) if i.lower() not in stopwords if i != ""]
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda x: -len(x[1])).take(1)
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda x:x[1]).flatMap(lambda x: set(x))
...
def norm(a):
    ...
    """
    return sum(a[k]*a[k] for k in a)**0.5
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum((amazonWeightsBroadcast.value[amazonRec][a])*(googleWeightsBroadcast.value[googleRec][a]) for a in tokens)

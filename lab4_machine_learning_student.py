...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    rating=IDandRatingsTuple[1]
    tupleSum = sum(i for i in rating)
    tupleCount = len(rating)
    return (IDandRatingsTuple[0],(tupleCount,tupleSum/float(tupleCount)))
...
def computeError(predictedRDD, actualRDD):
    ...
    return (float(totalError)/numRatings)**0.5
...
trainingAvgRating = (trainingRDD.map(lambda (a,b,c):c).reduce(lambda x,y:x+y))/float(trainingRDD.count())
...
myRatedMovies = [
     (0,993,4),(0,992,4),(0,991,4),(0,990,4),(0,994,4),(0,995,4),(0,996,4),(0,997,4),(0,998,4),(0,999,4),(0,90,4)
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda x: (0, x[0]))
                      .filter(lambda x: x[1] not in myRatedMovies))
